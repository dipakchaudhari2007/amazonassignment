...
public class Utility extends BaseTestCase{
	...
	public static void hideKeyboard() {
		if(driver.getPlatformName().trim().toLowerCase().contains("ANDROID")) {
			driver.hideKeyboard();
			/*if() {
	...
	public static void zoomIn(AppiumDriver<MobileElement> driver) {
		...
		//tAction0.press(scrWidth/4,scrHeight/4).moveTo(scrWidth/4,60).release();//press finger center of the screen and then move y axis
		//tAction1.press(scrWidth/4,(scrHeight/4)+40).moveTo(scrWidth/4,80).release();// press thumb slightly down on the center of the screen and then move y axis
